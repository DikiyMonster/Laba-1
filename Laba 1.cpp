...
// Последовательная обработка элементов файла(умножение на число)
...
void parallelProcessing(const string& filename,int exponent, int numThreads)
{
    ...
    // Ожидание замершения работы каждого потока
...
int main()
{
    ...
    // Задаём количество потоков для параллельной обработки
    int numThreads;
    ...
    cout << "Время многопоточной обработки: " << duration.count() << " ìñ" << endl;
